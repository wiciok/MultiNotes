...
namespace MultiNotes.XAndroid
{
    public class SlidingTabStrip : LinearLayout
    {
        ...
        public void OnViewPagerPageChanged(int position, float positionOfffset)
        {
            mSelectedPosition = position;
            mSelectionOffset = positionOfffset;
            this.Invalidate();
        }
        ...
        protected override void OnDraw(Canvas canvas)
        {
            ...
            int tabCount = this.ChildCount;
            int dividerHeightPx = (int)(Math.Min(Math.Max(0.0f, mDividerHeight), 1.0f) * height);
            SlidingTabScrollView.ITabColorizer tabColorizer
            ...
            if (tabCount > 0)
            {
                ...
                if (mSelectionOffset > 0.0f && mSelectedPosition < (tabCount - 1))
                {
                    ...
                    left = (int)(mSelectionOffset * nextTitle.Left
                            + (1.0f - mSelectedPosition) * left);
                    right = (int)(mSelectionOffset * nextTitle.Right
                            + (1.0f - mSelectedPosition) * right);
                }
                ...
                for (int i = 0; i < ChildCount; ++i)
                {
                    ...
                    mDividerPaint.Color = GetColorFromInteger(tabColorizer.GetDividerColor(color));
                    canvas.DrawLine(
                        child.Right,
                        separatorTop,
                        child.Right,
